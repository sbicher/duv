...
export class Race {
	...
	private IAULabel: string; // enum for the Label of the IAU (International Association of Ultrarunners); possible values: 'N' == 'None', 'B' == 'Bronze'
	private Startdate: string; // start date in the format 'yyyy-MM-dd'
	private Enddate: string; // start date in the format 'yyyy-MM-dd HH:mm:ss'
	private Year: number; // year of the race
	private Month: number; // month of the race (e.g. 4 == 'April')
	private Cupname: string; 
	private CupYear: string; 
}